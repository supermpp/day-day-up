					...
					if(';' != p[j]) //skip over comment
					{
						while(j < i && p[j]!='=') {
							...
							if('=' == p[j]) {
								...
								//if(strcmp(key,p+newline_start) ==0)
								{
									//find the key ok
									*key_s = newline_start;
									*key_e = j-1;
					...
					const char *value, const char *file)
{
	...
	if( -1 == sec_s)
	{
		if(0==file_size)
		{
			...
			snprintf(w_buf+file_size,100,"[%s]\n%s=%s\n",section,key,value);
		}
		else
		{
	...
	else if(-1 == key_s)
	{
		...
		snprintf(w_buf+sec_e,100,"%s=%s\n",key,value);
		snprintf(w_buf+sec_e+strlen(key)+strlen(value)+2,100,buf+sec_e, file_size - sec_e);
	}
	else
	{
